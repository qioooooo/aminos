...
namespace Microsoft.VisualBasic.CompilerServices
{
	...
	public sealed class ExceptionUtils
	{
		...
		internal static Exception MakeException1(int hr, string Parm1)
		{
			...
			if (hr > 0 && hr <= 65535)
			{
				text = Utils.GetResourceString((vbErrors)hr);
			}
			else
			{
				text = "";
			}
			int num = text.IndexOf("%1", StringComparison.OrdinalIgnoreCase);
			if (num >= 0)
			{
				text = text.Substring(0, num) + Parm1 + text.Substring(checked(num + 2));
			}
			return ExceptionUtils.VbMakeExceptionEx(hr, text);
		}
		...
		{
			bool flag;
			Exception ex = ExceptionUtils.BuildException(Number, sMsg, ref flag);
			if (flag)
			{
		...
		internal static Exception BuildException(int Number, string Description, ref bool VBDefinedError)
		{
			...
			if (Number == 3 || Number == 20 || Number == 94 || Number == 100)
			{
				return new InvalidOperationException(Description);
			}
			if (Number == 5 || Number == 448 || Number == 446 || Number == 449)
			{
			...
			if (Number == 57 || Number == 52 || Number == 54 || Number == 55 || Number == 58 || Number == 59 || Number == 61 || Number == 63 || Number == 67 || Number == 68 || Number == 70 || Number == 71 || Number == 74 || Number == 75)
			{
				return new IOException(Description);
			}
			if (Number == 76 || Number == 432)
			{
				return new FileNotFoundException(Description);
			}
			if (Number == 91)
