...
namespace System.Speech.Internal
{
	...
	{
		// Token: 0x06000047 RID: 71 RVA: 0x00005247 File Offset: 0x00004247
		private PhonemeConverter(PhonemeConverter.PhoneMap phoneMap)
		{
			this._phoneMap = phoneMap;
		}

		// Token: 0x1700000C RID: 12
		// (get) Token: 0x06000048 RID: 72 RVA: 0x00005256 File Offset: 0x00004256
		internal static PhonemeConverter UpsConverter
		{
			get
			{
				return PhonemeConverter._upsConverter;
			}
		}

		// Token: 0x06000049 RID: 73 RVA: 0x00005260 File Offset: 0x00004260
		internal static string ConvertPronToId(string pronunciation, int lcid)
		{
			PhonemeConverter phonemeConverter = PhonemeConverter.UpsConverter;
			foreach (PhonemeConverter.PhoneMap phoneMap in PhonemeConverter._phoneMaps)
			{
			...
			{
				throw new FormatException(SR.Get(SRID.EmptyPronunciationString, new object[0]));
			}
			return text;
		}

		// Token: 0x0600004A RID: 74 RVA: 0x000052C4 File Offset: 0x000042C4
		internal string ConvertPronToId(string sPhone)
		{
			...
			PhonemeConverter.PhoneId phoneId = new PhonemeConverter.PhoneId();
			while (i < length)
			{
				...
				phoneId._phone = text2;
				int num2 = Array.BinarySearch<PhonemeConverter.PhoneId>(this._phoneMap._phoneIds, phoneId, phoneId);
				if (num2 < 0)
				{
					throw new FormatException(SR.Get(SRID.InvalidPhoneme, new object[] { text }));
				}
				foreach (char c in this._phoneMap._phoneIds[num2]._cp)
				{
					stringBuilder.Append(c);
				}
				i = num;
				while (i < length && sPhone[i] == ' ')
				{
					i++;
				}
			}
		...
		// Token: 0x0600004B RID: 75 RVA: 0x000053CE File Offset: 0x000043CE
		internal static void ValidateUpsIds(string ids)
		{
			PhonemeConverter.ValidateUpsIds(ids.ToCharArray());
		}

		// Token: 0x0600004C RID: 76 RVA: 0x000053DC File Offset: 0x000043DC
		internal static void ValidateUpsIds(char[] ids)
		{
			foreach (char c in ids)
			{
				if (Array.BinarySearch<char>(PhonemeConverter._updIds, c) < 0)
				{
					throw new FormatException(SR.Get(SRID.InvalidPhoneme, new object[] { c }));
				}
			}
		}

		// Token: 0x0600004D RID: 77 RVA: 0x0000542C File Offset: 0x0000442C
		private static PhonemeConverter.PhoneMap[] DecompressPhoneMaps(PhonemeConverter.PhoneMapCompressed[] pmComps)
		{
			PhonemeConverter.PhoneMap[] array = new PhonemeConverter.PhoneMap[pmComps.Length];
			for (int i = 0; i < pmComps.Length; i++)
			{
				PhonemeConverter.PhoneMapCompressed phoneMapCompressed = pmComps[i];
				PhonemeConverter.PhoneMap phoneMap = (array[i] = new PhonemeConverter.PhoneMap());
				phoneMap._lcid = phoneMapCompressed._lcid;
				phoneMap._phoneIds = new PhonemeConverter.PhoneId[phoneMapCompressed._count];
				int num = 0;
				int num2 = 0;
				for (int j = 0; j < phoneMap._phoneIds.Length; j++)
				{
					phoneMap._phoneIds[j] = new PhonemeConverter.PhoneId();
					int num3 = 0;
					int num4 = num;
					while (phoneMapCompressed._phones[num4] != 0)
					{
						if (phoneMapCompressed._phones[num4] == 255)
						{
							num3++;
						}
						num4++;
					...
					{
						array2[k] = (char)phoneMapCompressed._phones[num++];
					}
					num += num3 + 1;
					phoneMap._phoneIds[j]._phone = new string(array2);
					phoneMap._phoneIds[j]._cp = new char[num3 + 1];
					for (int l = 0; l < phoneMap._phoneIds[j]._cp.Length; l++)
					{
						phoneMap._phoneIds[j]._cp[l] = phoneMapCompressed._cps[num2++];
					}
				}
			}
			return array;
		...
		private PhonemeConverter.PhoneMap _phoneMap;

		// Token: 0x04000087 RID: 135
		private static readonly PhonemeConverter.PhoneMap[] _phoneMaps = PhonemeConverter.DecompressPhoneMaps(PhonemeConverter._phoneMapsCompressed);

		// Token: 0x04000088 RID: 136
		private static PhonemeConverter.PhoneMapCompressed[] _phoneMapsCompressed = new PhonemeConverter.PhoneMapCompressed[]
		{
			new PhonemeConverter.PhoneMapCompressed(0, 207, new byte[]
			{
				...
				0, 65, 73, byte.MaxValue, byte.MaxValue, 0, 65, 78, byte.MaxValue, 0,
				65, 79, 0, 65, 79, 69, 0, 65, 79, 88,
				byte.MaxValue, byte.MaxValue, 0, 65, 80, 73, 0, 65, 83, 80,
				0, 65, 84, 82, 0, 65, 85, byte.MaxValue, byte.MaxValue, 0,
				65, 88, 0, 65, 88, 82, 0, 66, 0, 66,
				66, 0, 66, 72, 0, 66, 73, 77, 0, 66,
				86, 65, 0, 66, 86, 68, 0, 67, 0, 67,
				67, byte.MaxValue, byte.MaxValue, 0, 67, 67, 50, 0, 67, 67,
				75, 0, 67, 69, 78, 0, 67, 72, byte.MaxValue, byte.MaxValue,
				0, 67, 72, 50, 0, 67, 74, 0, 67, 84,
				0, 67, 86, 68, 0, 68, 0, 68, 69, 78,
				0, 68, 72, 0, 68, 73, 77, 0, 68, 82,
				0, 68, 88, 0, 68, 88, 82, 0, 68, 90,
				byte.MaxValue, byte.MaxValue, 0, 68, 90, 50, 0, 69, 0, 69,
				72, 0, 69, 72, 88, byte.MaxValue, byte.MaxValue, 0, 69, 73,
				byte.MaxValue, byte.MaxValue, 0, 69, 74, 67, 0, 69, 78, byte.MaxValue,
				...
				0, 73, 89, 88, byte.MaxValue, byte.MaxValue, 0, 74, 0, 74,
				67, byte.MaxValue, byte.MaxValue, 0, 74, 67, 50, 0, 74, 68,
				0, 74, 72, byte.MaxValue, byte.MaxValue, 0, 74, 72, 50, 0,
				74, 73, 77, 0, 74, 74, byte.MaxValue, byte.MaxValue, 0, 75,
				...
				78, byte.MaxValue, 0, 79, 73, byte.MaxValue, byte.MaxValue, 0, 79, 78,
				byte.MaxValue, 0, 79, 85, 0, 79, 87, 88, byte.MaxValue, byte.MaxValue,
				0, 79, 88, 0, 80, 0, 80, 65, 76, 0,
				80, 67, 75, 0, 80, 70, byte.MaxValue, byte.MaxValue, 0, 80,
				...
				84, 72, 0, 84, 82, 0, 84, 83, byte.MaxValue, byte.MaxValue,
				0, 84, 83, 50, 0, 84, 83, 82, byte.MaxValue, byte.MaxValue,
				0, 85, 0, 85, 72, 0, 85, 82, 0, 85,
				85, 0, 85, 87, 88, byte.MaxValue, byte.MaxValue, 0, 85, 89,
				88, byte.MaxValue, byte.MaxValue, 0, 86, 0, 86, 65, 0, 86,
			...
			new PhonemeConverter.PhoneMapCompressed(1028, 52, new byte[]
			{
			...
			new PhonemeConverter.PhoneMapCompressed(1031, 53, new byte[]
			{
			...
			new PhonemeConverter.PhoneMapCompressed(1033, 49, new byte[]
			{
				45, 0, 33, 0, 38, 0, 44, 0, 46, 0,
			...
			new PhonemeConverter.PhoneMapCompressed(1034, 35, new byte[]
			{
				45, 0, 33, 0, 38, 0, 44, 0, 46, 0,
			...
			new PhonemeConverter.PhoneMapCompressed(1036, 42, new byte[]
			{
				45, 0, 33, 0, 38, 0, 44, 0, 46, 0,
			...
			new PhonemeConverter.PhoneMapCompressed(1041, 102, new byte[]
			{
			...
			new PhonemeConverter.PhoneMapCompressed(2052, 422, new byte[]
			{
		...
		private static char[] _updIds = new char[]
		{
			'\u0001', '\u0002', '\u0003', '\u0004', '!', '.', 'a', 'b', 'c', 'd',
			'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n',
			'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x',
		...
		private static readonly PhonemeConverter _upsConverter = new PhonemeConverter(PhonemeConverter._phoneMaps[0]);

		// Token: 0x02000014 RID: 20
		private class PhoneMap
		{
			// Token: 0x0600004E RID: 78 RVA: 0x00005591 File Offset: 0x00004591
			internal PhoneMap()
			{
			}
			...
			internal PhonemeConverter.PhoneId[] _phoneIds;
		}

		// Token: 0x02000015 RID: 21
		private class PhoneId : IComparer<PhonemeConverter.PhoneId>
		{
			// Token: 0x0600004F RID: 79 RVA: 0x00005599 File Offset: 0x00004599
			internal PhoneId()
			{
			}

			// Token: 0x06000050 RID: 80 RVA: 0x000055A1 File Offset: 0x000045A1
			int IComparer<PhonemeConverter.PhoneId>.Compare(PhonemeConverter.PhoneId x, PhonemeConverter.PhoneId y)
			{
		...
		{
			// Token: 0x06000051 RID: 81 RVA: 0x000055B4 File Offset: 0x000045B4
			internal PhoneMapCompressed()
			{
			}

			// Token: 0x06000052 RID: 82 RVA: 0x000055BC File Offset: 0x000045BC
			internal PhoneMapCompressed(int lcid, int count, byte[] phoneIds, char[] cps)
			{
				this._lcid = lcid;
				this._count = count;
				this._phones = phoneIds;
				this._cps = cps;
			}
